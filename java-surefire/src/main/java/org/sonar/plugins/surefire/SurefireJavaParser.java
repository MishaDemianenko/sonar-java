...
public class SurefireJavaParser {
  ...
  private void save(UnitTestIndex index, SensorContext context) {
    ...
    Map<InputFile, UnitTestClassReport> indexByInputFile = consolidateResults(index.getIndexByClassname());
  ...
  private Map<InputFile, UnitTestClassReport> consolidateResults(Map<String, UnitTestClassReport> indexByClassname) {
    ...
    indexByClassname.forEach((className, index) -> {
      ...
      if (resource != null) {
        result.putIfAbsent(resource, new UnitTestClassReport());
        // in case of repeated/parameterized tests (JUnit 5.x) we may end up with tests having the same name
        index.getResults().forEach(r -> result.get(resource).add(r));
      } else {
